...
之前提到的Flannel[Host Gateway模式]()方案是不能跨二层网络，是因为它只能修改主机路由，Calico把改路由表的做法换成了标准的BGP路由协议。相当于在每个节点上模拟出一个额外的路由器，由于采用的是标准协议，Calico模拟路由器的路由表信息可以被传播到网络的其他路由设备中，这样就实现了在三层网络上的高速跨节点网络。
...
![](https://gcore.jsdelivr.net/gh/tenqaz/BLOG-CDN@main/20230603114636.png)
...
![](https://gcore.jsdelivr.net/gh/tenqaz/BLOG-CDN@main/20230604140738.png)
